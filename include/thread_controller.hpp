...
class ThreadController {
  public:
     ...
     * @param numThreads Number of producer threads to spawn. Defaults to hardware concurrency.
     * @param queueCapacity Maximum capacity of internal queues to apply backpressure.
     */
     explicit ThreadController (size_t numThreads = std::thread::hardware_concurrency(), size_t queueCapacity = 128);
     ...
     */
     void run(const std::vector<fs::path>& image_paths, Pipeline& pipeline, const std::string& output_dir, int iterations);

  private:
    /**
     * @brief Launches producer threads that pull tasks from pathQueue_ and push augmented images.
     ...
     */
     void launchProducers(const std::vector<fs::path>& image_paths, int iterations, Pipeline& pipeline);

     /**
     * @brief Launches consumer thread that saves augmented images from imageQueue_ to disk.
     * @param output_dir Directory to save images.
     */
    void launchConsumer(const std::string& output_dir);

    /**
     * @brief Waits for all producer threads to finish and signals consumer to stop.
     */
    void waitForCompletion();

    size_t numThreads_;                   ///< Number of producer threads.
    SafeQueue<fs::path> pathQueue_;      ///< Queue holding image paths to be processed.
    SafeQueue<Image> imageQueue_;         ///< Queue holding augmented images ready to save.
